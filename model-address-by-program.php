...
function selectAddressByProgram($programID) {
    try {
        ...
        $stmt = $conn->prepare("SELECT p.GymID, ProgramName, GymName, Address, City, State, ZipCode FROM project.program p join project.gym g on p.GymID = g.GymID WHERE p.ProgramID = ?");
